...
#2461214151
instructions = instructions + [0 for _ in range(20000)]
#print(instructions)
...
def turn(num, pos):
    ...
    if pos[2] == "^" and num == 1:
        return (pos[0]+1, pos[1],">")
    if pos[2] == "v" and num == 0:
        return (pos[0]+1,pos[1],">")
    if pos[2] == "v" and num == 1:
        return (pos[0]-1,pos[1],"<")
    if pos[2] == ">" and num == 0:
...
inputs_inverse ={'black':0, 'white':1} 
positions = set([(0,0)])
inter = 0
print(amp.instructions[:10])
while amp.instructions[amp.index] != 99:
    inter+=1
    amp.start()
    try:
        colors[(pos[0], pos[1])] = inputs[amp.outs[0]]
    except:
        break
    pos = turn(amp.outs[1], pos)
    positions.add((pos[0], pos[1]))
    if (pos[0], pos[1]) in colors:
        amp.phase = inputs_inverse[colors[(pos[0],pos[1])]]
    else:
        amp.phase = 0
...
yset = set([-p[1] for p in colors.keys()])
xset = set([p[0] for p in colors.keys()])
